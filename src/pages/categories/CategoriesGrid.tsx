    ...
    return (
        <div>
                ...
                {selectMode ?
                    <SelectActions
                        ...
                        checkAllHandler={(e: CheckboxChangeEvent) => {
                            setSelected(e.target.checked ? categories.map(cat => cat.id) : [])
                        }}
                        abortHandler={() => {
                            setSelected([]);
                            setSelectMode(false);
            ...
            <List
                ...
                dataSource={loading && categories.length === 0
                    ...
                    <List.Item style={{ margin: 0 }}>
                        {loading && categories.length === 0 ? (
                            ...
                            <Card style={{width: '100%'}}
                                  actions={[
                                      <Link to={`/categories/edit/${category.id}`}><EditOutlined key="edit" /></Link>,
                                      <ContextMenu
                                          menuItems={categoriesGridMenuItems}
                                          handlers={{
                                              delete: () => deleteCategoriesHandler([category.id]),
                                          }}
                                      />
                                  ]}
                            >
                                ...
                                <Space>
                                    ...
                                    <div>
                                        <Typography.Title level={4} style={{marginTop: 0}}>
                                            {category.title}&nbsp;&nbsp;
                                            <span style={{color: 'var(--ant-color-icon)'}}>({category.id})</span>
                                        </Typography.Title>
                                        <p style={{color: 'var(--ant-color-icon)'}}>
                                            Количество подкатегорий: {category.types.length}
                                        </p>
                                    </div>
                                </Space>
